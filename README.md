...
- ***[build_sylenth1_timbre_dashboard.py](build_sylenth1_timbre_dashboard.py)*** — builds the interactive Plotly dashboards for exploring the contents and distributions of the dataset JSONs. *Usage*: `python build_sylenth1_timbre_dashboard.py --in [TIMBRAL_DATASET].json --out [DASHBOARD].html`.
- ***[pca_audiocommons_analysis.py](pca_audiocommons_analysis.py)*** — runs the PCA analysis on the dataset; the PCA figures in Section 5.3 are produced by this script (scree plot with integer component ticks, annotated PC–descriptor heatmap on an observed-range colour scale, per-PC loadings, and the per-descriptor correlation grid now provided in the paper's Supplementary File). *Usage*: `python pca_audiocommons_analysis.py --in [TIMBRAL_DATASET].json --outdir [OUT_DIR]`.
- ***[gen_ac_hist.py](gen_ac_hist.py)*** — produces the AudioCommons descriptor histograms (Section 5.1, Figure 3); writes `AC_Hist.png`. *Usage*: `python gen_ac_hist.py` (input path set inside the script).
- ***[gen_param_ac_corr.py](gen_param_ac_corr.py)*** — produces the parameter–descriptor Pearson correlation heatmap with numbered columns (Section 5.2, Figure 5); writes `pearson_correlations.png` and the index-to-name key `pearson_correlations_column_key.csv`. *Usage*: `python gen_param_ac_corr.py`.
- ***[gen_pca_rand_vs_fact.py](gen_pca_rand_vs_fact.py)*** — produces the factory-vs-random PCA comparison figures (Supplementary File, Section S2); writes `fig_pca_factory_random_density_triptych.png`, `fig_pca_factory_random_overlay.png`, and the per-preset PC scores `pc_scores_factory_random.csv`. *Usage*: `python gen_pca_rand_vs_fact.py`.
...
The `baselines/` directory contains the implementations and released artefacts for everything reported in Sections 5.4 (convergent validity), 5.5 (pitch sensitivity), and 6 (baseline implementations) of the paper. The engineering map and headline results table live in [README_baselines.md](README_baselines.md).
...
- ***[baselines/regression/train_regression.py](baselines/regression/train_regression.py)*** — parameter→descriptor regression baselines (mean predictor, ridge, MLP); reproduces the Section 6 regression table (MLP reported as mean ± std over four training seeds — see the multi-seed section of [README_baselines.md](README_baselines.md)).
...
- ***[baselines/sensitivity/](baselines/sensitivity/)*** — convergent-validity analysis against librosa acoustic features (`convergent_validity.py`), multi-pitch re-rendering with the sensitivity heatmap and signed-deviation boxplots (`multipitch_render.py`, `multipitch_analyze.py`), the consolidated factory-vs-random parameter distributions figure (Figure 4; `param_distributions_consolidated.py`), and the pathology-audit candidate selector (`pathology_candidates.py`).
...
- ***[baselines/artifacts/cvae_checkpoint.pt](baselines/artifacts/cvae_checkpoint.pt)*** — trained CVAE weights (seed 0); `cvae_checkpoint_seed{1..3}.pt` are the additional training seeds behind the paper's mean ± std results.
...
- ***[baselines/artifacts/results/](baselines/artifacts/results/)*** — per-task CSVs and notes: render-parity check, regression metrics, inversion metrics (NN and CVAE, partial and full test set), multipitch renders and per-descriptor sensitivity (with std / median absolute deviation / signed range columns), per-seed and seed-aggregated metrics for the multi-seed experiments, convergent-validity Spearman correlations (overall and per preset), pathology-audit candidates, and accompanying `*_notes.md` files with discussion.
- ***[baselines/artifacts/figures/](baselines/artifacts/figures/)*** — all figure assets for the paper: `param_distributions_consolidated.png`, `convergent_validity_heatmap.png`, `multipitch_heatmap.png`, `multipitch_boxplots.png`, the regenerated PCA figure set under `pca_audiocommons_outputs/`, and per-seed MAE bar charts.
